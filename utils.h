...
const char title[] = "PF-Project, 22i-1008_H";
...
int point_1[4][2], point_2[4][2],shadow_copy[4][2];

//---Check Uncertain Conditions---//
bool anamoly(){
    for (int i=0;i<4;i++)
        if (point_1[i][0]<0 || point_1[i][0]>=N || point_1[i][1]>=M) //ensures that the blocks start from a particular position
            return 0;
        else if (gameGrid[point_1[i][1]][point_1[i][0]])  //preventing blocks from overwriting (coming on top of each other)
            return 0;
    return 1;
}
...
bool shadow_anamoly(){
    for (int i=0;i<4;i++)
    {
        if (shadow_copy[i][0]<0 || shadow_copy[i][0]>=N ||shadow_copy[i][1]>=M-1)
            return 0;
        if (gameGrid[shadow_copy[i][1]+1][shadow_copy[i][0]])
            return 0;
    }   
    return 1;
};
